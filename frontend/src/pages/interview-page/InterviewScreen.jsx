...
Keep it concise and actionable.`;
  ...
  async (text) => {
    ...
    // ---------------------------------------------------------
    // 1️⃣ AUTO PROGRESSION BETWEEN ROUNDS
    // ---------------------------------------------------------

    const introQ = chatMessages.filter(
      (m) => m.sender === "AI" && m.stage === "intro"
    ).length;

    const transQ = chatMessages.filter(
      (m) => m.sender === "AI" && m.stage === "transition"
    ).length;

    const techQ = chatMessages.filter(
      (m) => m.sender === "AI" && m.stage === "technical"
    ).length;
    ...
    if (interviewStage === "technical" && techQ >= 4) {
      const announce =
        "That concludes the technical round. We will now begin the coding round.";
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          console.error("Failed to fetch initial coding problem:", err);
          setError("Failed to generate coding problem. Try again.");
          return;
        }
        ...
        setChatMessages((prev) => [
          ...
          {
            sender: "AI",
            text: "Your first coding problem is ready. Open the Code Editor to solve it.",
            time: new Date().toLocaleTimeString(),
            stage: "coding",
          },
        ]);
    ...
    // ---------------------------------------------------------
    // 2️⃣ BLOCK USER ASKING FOR CODING OUTSIDE CODING ROUND
    // ---------------------------------------------------------
    const lowerMsg = messageToSend.toLowerCase();

    const userIsRequestingCoding =
      lowerMsg.includes("coding") ||
      lowerMsg.includes("coding problem") ||
      lowerMsg.includes("coding question") ||
      lowerMsg.includes("give me coding") ||
      lowerMsg.includes("generate a coding");

    if (userIsRequestingCoding && interviewStage !== "coding") {
      const refusal = `Coding questions come only in the coding round. We are currently in the ${interviewStage} round.`;
      ...
      textToSpeech(refusal);
      return;
    }

    // ---------------------------------------------------------
    // 3️⃣ ROLE DEVIATION BLOCKING
    // ---------------------------------------------------------
    const domainSwitchCommands = [
      "ask ui",
      "ask ux",
      "ask ml",
      "ask cloud",
    ...
    if (domainSwitchCommands.some((cmd) => lowerMsg.includes(cmd))) {
      const updated = deviationWarnings + 1;
      setDeviationWarnings(updated);

      const warning =
        updated >= 3
          ? `You have repeatedly attempted to deviate from the role ${role}. This interview is now ending.`
          : `You selected the role ${role}. Please do not deviate. (Warning ${updated}/3)`;
    ...
    // ---------------------------------------------------------
    // 4️⃣ FINAL ROUND LOGIC (AFTER CODING)
    // ---------------------------------------------------------
    if (interviewStage === "final") {
      const finalQuestionsAsked = chatMessages.filter(
        (m) => m.sender === "AI" && m.stage === "final"
      ).length;

      // After 2 wrap-up questions → END INTERVIEW automatically
      if (finalQuestionsAsked >= 2) {
        const closingText =
          "Thank you for your time today. This concludes your interview. I will now generate your interview report.";
        ...
        setTimeout(() => handleEndInterview(), 2500);
        return;
      }
    }

    // ---------------------------------------------------------
    // 5️⃣ NORMAL AI RESPONSE PROCESS (FOR ALL ROUNDS)
    // ---------------------------------------------------------
    ...
    try {
      ...
      if (interviewStage === "intro")
        systemInstruction =
          "You are in the intro HR round. Ask only HR intro questions.";
      else if (interviewStage === "transition")
        systemInstruction =
          "You are transitioning to technical. Ask conceptual pre-technical questions.";
        ...
        systemInstruction =
          "You are in the coding round. DO NOT generate coding problems. The system handles coding questions.";
      else if (interviewStage === "final")
        systemInstruction =
          "You are in the final HR wrap-up round. Ask closing interview questions such as strengths, weaknesses, goals, why hire you, etc. No technical or coding questions.";
